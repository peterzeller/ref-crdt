...
lemma unique_id_effector_once:
  assumes wf: "wellFormed_impl E"
    ...
    and uid_from_event: "\<And>eff e opr s uid . \<lbrakk>eff \<in> set (snd (generator_impl opr e s)); get_uid eff = Some uid\<rbrakk> 
           \<Longrightarrow> uid = e"             
    and uid_unique: "\<And>opr e s. distinct (List.map_filter get_uid (snd (generator_impl opr e s)))"
    and i: "get_uid (effectors!i) = Some uid"
  shows "\<exists>!i. \<exists>!k. exec_order!i = (uid, k) \<and> get_uid (effectors!i) = Some uid"
  find_theorems intro
proof -

  obtain k where k_def: "exec_order!i = (uid, k)"
    by (metis effector_uid_from_event effectors_def i i'_len local.wf snapshot_valid uid_from_event valid_event_seq)



  show "\<exists>!i. \<exists>!k. exec_order ! i = (uid, k) \<and> get_uid (effectors ! i) = Some uid"
  proof
    show "\<exists>!k. exec_order ! i = (uid, k) \<and> get_uid (effectors ! i) = Some uid"
    proof

      show "exec_order ! i = (uid, k) \<and> get_uid (effectors ! i) = Some uid"
        by (simp add: i k_def)


      show "k' = k" if "exec_order ! i = (uid, k')  \<and> get_uid (effectors ! i) = Some uid"  for  k'
        using k_def that by auto
    qed

    show "i' = i" if c: "\<exists>!k. exec_order ! i' = (uid, k) \<and> get_uid (effectors ! i') = Some uid" for  i'
    proof -
      from c
      obtain k' where "exec_order ! i' = (uid, k')" by force

      have "get_uid (effectors ! i') = Some uid"
        using that by blast


      (* must be coming from same event, different indexes *)
      have "\<exists>opr s. effectors ! i \<in> set (snd (generator_impl opr uid s))"
        using `i<length effectors` effectors_def wf
        apply (auto simp add: wellFormed_impl_def wellFormed_def wf_generator_def compose_forward_def get_effectors_def)

        sorry

      from this
      obtain effs opr s where effs_def: "effs = snd (generator_impl opr uid s)" and "effectors ! i \<in> set effs" (* and "effectors ! i' \<in> set effs" *)
        by force

      have "effectors ! i' \<in> set effs"
        using `get_uid (effectors ! i') = Some uid` (* Same uid, so must be generated by same event *)
        sorry

      obtain j where "j < length effs" and "effs ! j = ???"
        sorry
      show "i' = i"
        sorry
    qed
...
qed
 ...
 (* use assumption to get contradiction *)
